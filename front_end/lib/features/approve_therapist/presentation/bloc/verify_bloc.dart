// lib/features/approve_therapist/presentation/bloc/verify_bloc.dart
...
class VerifyBloc extends Bloc<VerifyEvent, VerifyState> {
  ...
  Future<void> _onLoadTherapists(
      LoadTherapistsEvent event, Emitter<VerifyState> emit) async {
    emit(VerifyLoading());
  ...
  Future<void> _onApproveTherapist(
      ApproveTherapistEvent event, Emitter<VerifyState> emit) async {
    emit(VerifyLoading());
  ...
  Future<void> _onRejectTherapist(
      RejectTherapistEvent event, Emitter<VerifyState> emit) async {
    emit(VerifyLoading());
    final result = await rejectUsecase(
        RejectTherapistParams(id: event.id, reason: event.reason));
    emit(result.fold(
      (failure) => VerifyError(message: failure.message),
